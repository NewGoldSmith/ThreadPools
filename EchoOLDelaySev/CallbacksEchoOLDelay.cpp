...
//			MyTRACE(("SevOL Front Sent:" + pSocket->WriteBuf).c_str());
	...
	VOID Delay100msecSendCB(PTP_CALLBACK_INSTANCE Instance, PVOID Context, PTP_TIMER Timer)
	{
	...
	BOOL Send(SocketContext* pSocket)
	{
		...
		if (!(pTPTimer = CreateThreadpoolTimer(Delay100msecSendCB, pSocket, &*pcbe)))
		{
			DWORD Err = GetLastError();
			stringstream  ss;
			ss << "EchoOLDelaySev. Send. send Code: " << Err << " LINE: " << __LINE__ << "\r\n";
			std::cerr << ss.str();
			MyTRACE(ss.str().c_str());
			return FALSE;
		}
